...
""" Hyperparameters """
gamma = 0.99
max_epsilon   = 0.01  # 1st round is 0.1, After 5170 episodes, reduce to 0.01
min_epsilon   = 0.01
epsilon_decay = 5000
batch_size    = 128
...
def lane_potential(agent_pos, next_agent_pos, goal_pos):
    ...
    return (manhattan_distance(next_agent_pos, goal_pos) - manhattan_distance(agent_pos, goal_pos)) / (height + width) * 2

def crash_penalty(done, reward):
...
def compute_dqn_loss(state_action_values, target, states, actions, rewards, next_states, dones):
    ...
    NepLogger.add_scalar('average predicted Q value', torch.mean(state_action_values).data)
    ...
    NepLogger.add_scalar('average target Q value', torch.mean(expected_state_action_values).data)
    ...
    def _sample(self, batch_size, buffer_list):
        ...
        for i, idx in enumerate(sample_idxes):
            ...
            if not buffer_list[idx].done:
                rewards[i, ...] = buffer_list[idx].reward + lane_potential(buffer_list[idx].state[1],
                    buffer_list[idx].next_state[1], buffer_list[idx].state[2]) + \
                        crash_penalty(buffer_list[idx].done, buffer_list[idx].reward)
            else:
...
class DQfDAgent(Agent):
    ...
    def optimize(self, optimizer, expert_phase=False):
        ...
        NepLogger.add_scalar('J_E loss', J_E.item())
        NepLogger.add_scalar('J_DQ loss', J_DQ.item())
        NepLogger.add_scalar('Total loss', loss.item())
    ...
    def record_episode_train(self, episode):
        ...
        k = 4   # Hyperparameter for the goal sampling rate 
